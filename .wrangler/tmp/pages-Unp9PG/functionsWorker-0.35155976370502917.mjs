...
var init_utils = __esm({
  "../../../.nvm/versions/node/v22.18.0/lib/node_modules/wrangler/node_modules/unenv/dist/runtime/_internal/utils.mjs"() {
    init_functionsRoutes_0_35111789799490367();
...
async function onRequest3(context2) {
  ...
  try {
    ...
    const apiKey = env2.OPENAI_API_KEY;
    if (!apiKey) {
      return new Response(JSON.stringify({ error: "OPENAI_API_KEY not set" }), { status: 500, headers: HEADERS3 });
    }
    ...
    try {
      ...
      const message = error3?.error?.message || error3?.message || "OpenAI request failed";
      return new Response(JSON.stringify({ error: message }), { status: 502, headers: HEADERS3 });
    }
    const text = parseOpenAIText(response);
    if (!text) {
      console.error("OpenAI custom prompt returned no text");
      return new Response(JSON.stringify({ error: "Ingen text genererades av OpenAI" }), { status: 502, headers: HEADERS3 });
    }
    const lines = text.split("\n");
    const firstLine = lines.shift() || "";
    const title2 = firstLine.replace(/^\s*#+\s*/, "").trim().slice(0, 160);
    const content = lines.join("\n").trim() || firstLine.trim();
    const sources = parseOpenAISources(response);
    return new Response(JSON.stringify({ title: title2, content, sources }), { status: 200, headers: HEADERS3 });
  } catch (e) {
    console.error(e);
    return new Response(JSON.stringify({ error: "Generation failed" }), { status: 500, headers: HEADERS3 });
  }
}
var HEADERS3, OPENAI_PROMPT2;
...
// ../.wrangler/tmp/pages-Unp9PG/functionsRoutes-0.35111789799490367.mjs
var routes;
var init_functionsRoutes_0_35111789799490367 = __esm({
  "../.wrangler/tmp/pages-Unp9PG/functionsRoutes-0.35111789799490367.mjs"() {
    init_image_generate();
    init_news();
    init_news_generate();
...
// ../.wrangler/tmp/bundle-TYi5HD/middleware-loader.entry.ts
init_functionsRoutes_0_35111789799490367();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// ../.wrangler/tmp/bundle-TYi5HD/middleware-insertion-facade.js
init_functionsRoutes_0_35111789799490367();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
//# sourceMappingURL=functionsWorker-0.35155976370502917.mjs.map
