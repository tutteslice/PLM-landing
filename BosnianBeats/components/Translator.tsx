...
const Translator: React.FC = () => {
  ...
  useEffect(() => {
    ...
    debounceTimerRef.current = setTimeout(async () => {
      try {
        const result = await translateToBosnian(inputText);
        setResults(result);
      } catch (error) {
  ...
  const playAudio = async (textToSpeak: string) => {
    ...
    try {
      ...
      const base64Audio = await speakBosnian(textToSpeak);
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="mt-8 p-6 bg-slate-800/30 rounded-2xl border border-slate-700/50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
          <div className="space-y-2">
            <p className="text-slate-400"><strong className="text-purple-400 uppercase text-[10px] tracking-widest block">Natural</strong> This is the default. It's how people actually talk at festivals, parties, or gatherings.</p>
          </div>
          <div className="space-y-2">
