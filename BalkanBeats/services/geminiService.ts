...
export async function translateToCroatian(text: string): Promise<TranslationResponse> {
  try {
    const response = await fetch('/api/balkan-beats-translate', {
      method: 'POST',
      headers: {
...
export async function speakCroatian(text: string): Promise<string | undefined> {
  try {
    const response = await fetch('/api/balkan-beats-speak', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
